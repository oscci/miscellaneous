...
phase2t <-c(161,316) #this phase has inverted U 
...
plot(alltime,truecurve,type='n',ylim=c(-.4,.4),main='True MMN with no error') #create frame to plot lines in 
...
Two weights are specified: wt1 is used for previous datapoint, and (1-wt1) is weighting for the random noise. wt2 is weighting for true curve: if this is zero, then we have a flat line; if it is high, then we see the effect of the true curve clearly.
...
#Next we set up a dataframe to hold the averaged ERP for each subject: each row is one timepoint, and there a column for each subject
subresults<-data.frame(matrix(NA,nrow=length(alltime),ncol=nsub+1))
subresults[,1]<-alltime #column 1 has the time values

#Another dataframe stores the MMN values for each subject
...
png("subs.png") #This plot too big for plot pane, so write to png: 4 x 4 grid of plots 
par(mfrow=c(4,4), mai = c(.5, 0.1, 0.1, 0.1)) #mai avoids big white spaces between plots
...
You should be able to see the impact of double-dipping: it makes a nonsense of the statistical test, because the data have been selected to be extreme.
...
Within a subject, signals from different electrodes tend to be correlated; we will model this by simulating one function per subject, but then adding noise to this for each simulated electrode. For simplicity we will use values of truecurve etc as in demonstration 1.
...
makemanywaves <- function(timepts,wt1,wt2,wt3,truecurve,nelec) {
  ...
  for (j in 2:timepts){ 
    for (e in 1:nelec){
      ...
      #work out next timepoint on basis of previous timepoint, randomness plus true curve
      mywavedf[j,e]<-wt1*mywavedf[j-1,e]+(1-wt1)*rnorm(1)+wt2*truecurve[j]+wt3*rnorm(1)
    }
  }
  return(mywavedf)
...
Now we will use this function to generate 6 electrode values for each of our subjects. Subjects, who don't actually differ, will be subdivided into 2 groups and the grand average waveforms will be plotted.
...
wt2 <- .5 #term that affects SNR - i.e. ability to detect true waveform influence
